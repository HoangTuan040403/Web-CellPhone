...
namespace Doanphanmem.Admin.Controllers
{
    public class ThongKeController : Controller
    {
        ...
        public int TongDonHuy(int khachhangid)
        {

            var tongDonHuy = db.DONDATHANGs.Count(i => i.HuyDon == true && i.MaKH == khachhangid);
            return tongDonHuy;
        }

        public int TongDonDaHuy()
        {

            var tongDonHuy = db.DONDATHANGs.Count(i => i.HuyDon == true );
            return tongDonHuy;
        }
        public decimal TinhTongTien()
        {
            var dONDATHANGs = db.DONDATHANGs.Sum(i=>i.Trigia);
             return dONDATHANGs.Value;
        }
        public int TinhSoLuong()
        {
            var dONDATHANGs = db.DONDATHANGs.Count();
             return dONDATHANGs;
        }
        public int TinhSLKH()
        {
            var khachhang = db.DONDATHANGs.AsQueryable().Select(dh => dh.MaKH).Distinct().ToList();
        ...
        public ActionResult Chart_sort(DateTime? fromDate, DateTime? toDate)
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                var doanhThu = db.DONDATHANGs.AsQueryable()
            ...
            ViewBag.TongDH = TinhSoLuong() - TongDonDaHuy();
            ViewBag.TongKH = TinhSLKH();
            ViewBag.TwelveMonthsData = twelveMonthsData;
            //ViewBag.TongDonHuy = TongDonDaHuy();

            //tôp 10 khách hàng
            var top10KhachHang = db.DONDATHANGs
                .GroupBy(dh => dh.MaKH)
                .Select(g => new
                {
                    MaKH = g.Key,
            ...
            {
                var khachHangDetail = db.KhachHangs.Find(khachHang.MaKH);
        ...
        public ActionResult Top10KhachHangMuaNhieuNhat()
        {
            var top10KhachHang = db.DONDATHANGs
               ...
               .Select(g => new
               {
                   MaKH = g.Key,
                   tenkh = g.FirstOrDefault().KhachHang.TenKH,
                   SoDienThoai = g.FirstOrDefault().KhachHang.sdt,
                   TongTienMua = g.Sum(x => x.Trigia ?? 0),
                   SoLuongDonHang = g.Count()
               })
               .OrderByDescending(kh => kh.TongTienMua)
            ...
            foreach (var khachHang in top10KhachHang.Where(kh => kh.MaKH != null))
            {
                ...
                if (khachHangDetail != null)
                {
                    var topKhachHangModel = new TopKhachHangModel
                    {
                        ...
                        tenkh = khachHang.tenkh,
                        TongTienMua = khachHang.TongTienMua,
                        SoLuongDonHang = khachHang.SoLuongDonHang - TongDonHuy(khachHang.MaKH.Value)
                    };
                    top10KhachHangDetails.Add(topKhachHangModel);
                }
            }
